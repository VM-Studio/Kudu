...
export default function PerfilPage() {
  ...
  // === IMÁGENES (FEED CUADRADO, PROLIJO) ===
  // Opción simple por secuencia: /public/feed/1.jpg ... /feed/N.jpg
  const TOTAL_POSTS = 12; // ajustá según lo que tengas
  const FALLBACK_POSTS = Array.from({ length: TOTAL_POSTS }, (_, i) => `/feed/${i + 1}.jpg`);
  const [posts, setPosts] = useState<string[]>(FALLBACK_POSTS);

  // Si querés listar por archivo, creá public/data/posts.json: ["1.jpg","2.jpg",...]
  useEffect(() => {
    fetch("/data/posts.json")
      .then((r) => (r.ok ? r.json() : Promise.reject()))
      .then((arr: string[]) => setPosts(arr.map((n) => `/feed/${n}`)))
      .catch(() => {});
  }, []);

  // === VIDEOS (VERTICALES / REELS 9:16) ===
  // Cargamos lista desde public/data/videos.json: ["video-1.mp4", ...]
  const [videos, setVideos] = useState<string[]>([]);
  useEffect(() => {
    fetch("/data/videos.json")
      .then((r) => (r.ok ? r.json() : Promise.reject()))
      .then((arr: string[]) => setVideos(arr.map((n) => `/videos/files/${n}`)))
      .catch(() => {
        // fallback si aún no creaste el JSON
        setVideos([
          "/videos/files/video-1.mp4",
          "/videos/files/video-2.mp4",
          "/videos/files/video-3.mp4",
        ]);
      });
  }, []);
  ...
  <h1
    className="text-4xl md:text-5xl font-extrabold tracking-tight text-slate-900 font-[Montserrat_Alternates]"
  >
    ...
    Recursos descargables, publicaciones y videos informativos.
        ...
        <div className="mx-auto mt-8 w-full max-w-5xl">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 rounded-3xl border border-zinc-200 bg-white px-5 py-4 shadow-[0_8px_28px_-16px_rgba(2,6,23,0.12)]">
            ...
            {/* Botón derecha */}
            <a
              href="/detalle.pdf"   // ← ajustá el nombre si es distinto
              download
              className="inline-flex items-center gap-2 rounded-xl bg-[#63798a]/10 px-5 py-3 text-[#63798a] font-semibold hover:bg-[#63798a] hover:text-white transition"
              aria-label="Descargar catálogo en PDF"
            >
          ...
          <ul className="flex gap-10">
            <li>
              ...
              >
                Publicaciones
                <span
                  className={`absolute left-0 -bottom-[1px] h-[2px] bg-[#647a8b] transition-all ${
                    tab === "posts" ? "w-full" : "w-0"
                  }`}
                />
              </button>
            </li>
        ...
        <div className="mt-8">
          {tab === "posts" ? (
            // === FEED: CUADRADOS IGUALES (como tu segunda imagen) ===
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {posts.map((src, i) => (
                <button
                  key={src + i}
                  onClick={() => setModal({ open: true, type: "image", src })}
                  className="group relative block overflow-hidden rounded-3xl ring-1 ring-black/5 bg-slate-200
                             aspect-square shadow-[0_12px_30px_-14px_rgba(0,0,0,0.25)]"
            ...
            // === VIDEOS: VERTICALES (rectángulos parados) 9:16 ===
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {videos.map((file, i) => (
                <button
                  key={file + i}
                  onClick={() => setModal({ open: true, type: "video", src: file })}
                ...
                >
                  {/* Preview: el propio video, vertical, cubriendo bien */}
                  <video
                    src={file}
                    muted
                    loop
      ...
      {modal.open && (
        <div
          ...
          className="fixed inset-0 z-[60] bg-black/70 backdrop-blur-[1px] grid place-items-center p-4"
          role="dialog"
          aria-modal="true"
        >
          <div onClick={(e) => e.stopPropagation()} className="relative">
            ...
            {modal.type === "image" && (
              <img
                ...
                alt="imagen ampliada"
                draggable={false}
                className="block max-w-[92vw] max-h-[85vh] object-contain rounded-xl shadow-2xl"
              />
            )}
